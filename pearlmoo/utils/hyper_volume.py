...
class _HyperVolume:
    ...
    def hvRecursive(self, dimIndex, length, bounds):
        ...
        if length == 0:
            ...
            p = sentinel
            q = p.prev[dimIndex]
            #print('sentinel',p,"cargo:",q.cargo,type(q.cargo),'ignore',q.ignore)
            q.cargo = list(q.cargo)
            while q.cargo != None:
                if q.ignore < dimIndex:
                    q.ignore = 0
                q = q.prev[dimIndex]
                if isinstance(q.cargo,numpy.ndarray):#Paul type error
                    q.cargo = list(q.cargo)
